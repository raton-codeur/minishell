...
/*   Updated: 2024/06/14 19:50:33 by qhauuy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "parser.h"

static void	build_tree_pipe(t_tree **tree, t_list *node, t_data *data)
{
	t_list	*tokens;
	t_tree	*left;
	t_tree	*right;
	...
	if (list_node_index(tokens, node) == 0)
		return (syntax_error(data, SYNTAX, "|"));
...
static void	build_tree_broket(t_tree **tree, t_list *node, t_data *data)
{
	...
	if (next.node == NULL)
		return (syntax_error(data, SYNTAX, current.content));
	else if (next.type != T_WORD)
		return (syntax_error(data, SYNTAX, next.content));
	new_tree(data);
	data->tree->content = copy_tokens(node, 1, data);
